...
static esp_err_t capture_handler(httpd_req_t *req) {
  ...
  if (fb->format == PIXFORMAT_JPEG) {
    ...
    while (offset < fb->len) {
      size_t send_size = (fb->len - offset < chunk_size) ? (fb->len - offset) : chunk_size;
      res = httpd_resp_send_chunk(req, (const char *)fb->buf + offset, send_size);
      if (res != ESP_OK) {
...
static esp_err_t sd_list_handler(httpd_req_t *req) {
  ...
  while (file) {
    if (!file.isDirectory()) {
      ...
      // file.name() 可能返回完整路径如 /data/IMG_xxx.jpg，只取纯文件名
  ...
  Serial.printf("列出 %d 张图片\n", count);
  return httpd_resp_send(req, response.c_str(), response.length());
}
// ... existing code ...
// 接口 2: GET /image?file=xxx.jpg
// 读取 SD 卡里指定图片并返回
static esp_err_t sd_image_handler(httpd_req_t *req) {
  char query[256];
  char filename[128];
  ...
  // 使用 4KB 小块读取，避免内存不足，每 10 次让一次 CPU
  ...
  while (sent < total) {
    size_t to_read = min(chunk_size, total - sent);
    size_t read_len = file.read(buffer, to_read);
    if (read_len == 0) break;

    if (httpd_resp_send_chunk(req, (const char *)buffer, read_len) != ESP_OK) {
      Serial.println("发送 chunk 失败!");
      free(buffer);
      file.close();
      return ESP_FAIL;
    }
  ...
  Serial.printf("文件发送完成: %s (%d 字节)\n", path.c_str(), sent);
  return ESP_OK;
}

// 工具函数：生成带时间戳的文件名
static String get_timestamp_filename() {
  struct timeval tv;
  gettimeofday(&tv, NULL);
  struct tm timeinfo;
  localtime_r(&tv.tv_sec, &timeinfo);
  char buf[64];
  strftime(buf, sizeof(buf), "IMG_%Y%m%d_%H%M%S.jpg", &timeinfo);
  return String(buf);
}

// 接口 3: GET /save
// 拍一张照片保存到 SD 卡，返回保存的文件名
static esp_err_t sd_save_handler(httpd_req_t *req) {
  ...
  String filename = get_timestamp_filename();
  String path = String("/data/") + filename;
  Serial.printf("保存到: %s\n", path.c_str());
  ...
  // 分块写入 SD 卡，4KB 一块，每 40 块让一次 CPU（喂看门狗）
  size_t written = 0;
  size_t chunk_size = 4096;
  int yield_counter = 0;
  while (written < fb->len) {
    size_t to_write = min(chunk_size, fb->len - written);
    size_t w = file.write(fb->buf + written, to_write);
    if (w == 0) {
      Serial.println("写入 SD 卡失败!");
      break;
    }
    written += w;
    yield_counter++;
  ...
  // 先保存 fb->len，因为 esp_camera_fb_return 后 fb 会被释放
  size_t fb_len = fb->len;
  file.close();
  esp_camera_fb_return(fb);

  if (written != fb_len) {
    Serial.printf("写入不完整! 需要写 %d, 实际写 %d，删除损坏文件\n", fb_len, written);
    SD_MMC.remove(path.c_str());
    httpd_resp_send_500(req);
    return ESP_FAIL;
  }

  Serial.printf("保存成功: %s (%d 字节)\n", filename.c_str(), fb_len);

  httpd_resp_set_type(req, "application/json");
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");

  String response = "{\"saved\":true,\"file\":\"" + filename + "\"}";
  return httpd_resp_send(req, response.c_str(), response.length());
}
...
void startCameraServer() {
  ...
  httpd_uri_t sd_image_uri = {
    .uri = "/image",
    .method = HTTP_GET,
    .handler = sd_image_handler,
    .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
    ,
    .is_websocket = true,
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &sd_image_uri);
