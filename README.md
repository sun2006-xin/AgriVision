...
阶段 1 已完成稳定运行基线，阶段 2 的架构与边界已完成收口，阶段 3 已完成可复现的算法评估与现场闭环基础；阶段 47–49 已补齐数据 provenance 审计、当前分类器的自动预测清单生成、跨光照/设备差异和现场阈值分析，真实标注集上的模型指标仍待验收。阶段 4–46 持续完善了离线队列、HTTP/MQTT 同步、连接生命周期、错误分类、脱敏状态和本机回归。当前公开分支追踪五个小型运行时权重，HuggingFace 缓存仍不进入 Git；首次使用前请按文档准备缓存和替代模型文件。
