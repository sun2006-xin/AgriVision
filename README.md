...
阶段 1 已完成基础可用性收敛，阶段 2 已完成工程化基础：修复 System B 调用 System A `/report` 时的上传字段契约，参数 API 增加后端白名单、有限数值和范围校验；新增 `/health/live`、`/health/ready`、请求 ID/耗时日志和原子配置写入，并补充 6 个回归测试。模型权重默认保留在本地，不随 Git 仓库分发；首次使用前请按文档准备模型文件。
