...
## 阶段 47：算法评估证据链审计（本机已完成，真实数据待验收）

- 在保持旧版 `schema_version=1` 评估清单兼容的前提下，增加显式 `--strict-provenance` 模式，要求数据集身份、模型版本与权重哈希、样本相对路径、样本哈希、采集组和已复核标注来源。
- 增加 `system_a/core/dataset_audit.py` 与 `tools/audit_evaluation_dataset.py`，可在不上传图片的情况下检查安全路径、文件存在性、普通文件类型、SHA-256 一致性、重复内容、切分泄漏和六个维度的覆盖分布。
- 评估 CLI 在严格证据模式下先执行审计再计算指标；文件校验失败不会继续生成看似成功的 Accuracy、校准或现场 FP/FN 报告。
- 新增阶段测试和 CI 严格 fixture；当前仓库仍没有真实标注图片集，因此本阶段完成的是可复核入口，不宣称真实模型指标已达标。

## 阶段 48：分类评估自动推理闭环（本机已完成，真实数据待验收）

- 新增 `system_a/core/classifier_inference.py`，集中维护 System A 分类器的类别顺序、224×224/ImageNet 预处理、logit→softmax、概率向量和不确定性证据；线上 `/predict` 与离线 runner 共用该实现。
- 新增 `tools/generate_prediction_manifest.py`，从没有预测字段的严格标注清单读取真实图片，验证文件哈希和模型权重哈希后，生成可直接交给 `evaluate_predictions.py` 的严格预测清单。
- runner 拒绝缺图、错哈希、模型版本不一致、类别数量/顺序不匹配和非有限模型输出；输出仍明确标记为未经校准分数。
- 新增阶段测试；由于仓库没有真实标注图片集，本阶段验证的是推理链路和失败边界，不发布真实 Accuracy、校准或现场 FP/FN 结果。

## 阶段 49：跨环境鲁棒性与运营阈值分析（本机已完成，真实数据待验收）

- `evaluation.py` 新增 `robustness.lighting/device`，把各切片的支持数、Accuracy、宏 F1、最差切片和 gap 作为显式输出，并明确这是非配对差异，不冒充跨光照实验。
- 新增 `field_threshold_curve`，在完整概率向量上计算现场筛查阈值对应的 FP/FN/FPR/FNR，CLI 支持自定义阈值列表；没有概率向量时保持不可用，不从 argmax 反推阈值证据。
- 新增阶段测试和旧链路回归；真实田间/多设备/多光照标注集仍缺失，因此本阶段没有发布业务门槛达标结论。
...
## 阶段 44：MQTT 手动同步错误码脱敏（本机已完成）

- 手动同步 503 响应使用固定 `mqtt_not_configured` 和 `mqtt_connection_failed` 错误码，区分配置缺口与连接故障。
- 响应不返回异常原文、broker 地址、用户名、密码或文件路径，便于前端和运维安全地处理故障类别。
- 本阶段只覆盖软件错误分类，不代表公网 broker、ACL、证书轮换或 ESP32 现场验收。

## 阶段 45：离线缓存文件系统并发保护（本机已完成）

- 缓存读取和 trim 统一通过安全文件枚举器，跳过目录等非普通文件，并容忍文件在枚举后被并发删除或替换。
